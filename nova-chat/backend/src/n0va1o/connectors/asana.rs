...
impl Connector for AsanaConnector {
    fn info(&self) -> ConnectorInfo {
        ConnectorInfo {
            id: "n0va1o.asana".to_string(),
            name: "Asana".to_string(),
            category: "Project Management".to_string(),
            description: "Sync with Asana — task creation, project updates, milestone tracking".to_string(),
            icon_url: None,
            auth_type: AuthType::OAuth2,
            capabilities: vec!["bidirectional_sync".to_string(), "task_creation".to_string(), "project_updates".to_string(), "milestone_tracking".to_string()],
            status: ConnectorStatus::Available,
        }
    }
    async fn connect(&self, user_id: &str, config: HashMap<String, String>) -> Result<String, ConnectorError> { let _ = (user_id, config); Ok(format!("asana_conn_{}", uuid::Uuid::new_v4().to_string().split('-').next().unwrap_or(""))) }
    ...
    async fn handle_webhook(&self, _: &[u8], _: &str) -> Result<SyncEvent, ConnectorError> { Ok(SyncEvent { event_id: format!("evt_{}", uuid::Uuid::new_v4().to_string().split('-').next().unwrap_or("")), connector_id: self.info().id, user_id: "unknown".to_string(), event_type: "task_completed".to_string(), payload: serde_json::json!({}), timestamp: chrono::Utc::now().to_rfc3339() }) }
