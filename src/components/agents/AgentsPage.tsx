      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {agents.map((agent) => (
          ...
          >
            ...
            <p className="text-xs text-surface-700 mb-3 line-clamp-2">{agent.description}</p>

            {/* Metrics */}
            <div className="grid grid-cols-3 gap-2 mb-3">
              <div className="text-center p-1.5 rounded-lg bg-surface-200/30">
                <p className="text-xs font-semibold text-surface-950">{formatNumber(agent.dailyActions)}</p>
                <p className="text-[10px] text-surface-600">Actions</p>
              </div>
              <div className="text-center p-1.5 rounded-lg bg-surface-200/30">
                <p className="text-xs font-semibold text-surface-950">{Math.round(agent.successRate * 100)}%</p>
                <p className="text-[10px] text-surface-600">Success</p>
              </div>
              <div className="text-center p-1.5 rounded-lg bg-surface-200/30">
                <p className="text-xs font-semibold text-surface-950">{formatLatency(agent.avgLatency)}</p>
                <p className="text-[10px] text-surface-600">Latency</p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-surface-300/20">
              ...
              <button
                ...
                className="p-1.5 rounded-lg hover:bg-surface-300/30 text-surface-600 hover:text-surface-900 transition-colors"
              >
      ...
      <Modal open={!!selectedAgent} onClose={() => setSelectedAgent(null)} size="lg" title={selectedAgent?.name || ''} subtitle={selectedAgent?.type}>
        {selectedAgent && (
          <div className="space-y-4">
            ...
            <div>
              <h4 className="text-xs font-semibold text-surface-900 mb-2">Requires Approval For</h4>
              <div className="flex flex-wrap gap-1.5">
            ...
            <div className="flex justify-end gap-2 pt-2">
              ...
              <Button variant="danger" onClick={() => toggleAgentStatus(selectedAgent.id)}>
