...
class ProductionGateway {
    ...
    setupAuthRoutes() {
        this.app.get('/v1/auth/:provider', async (req, res) => {
            try {
                ...
                const state = oauthService.generateState(agent_id, provider);
        ...
        this.app.get('/v1/auth/callback/:provider', async (req, res) => {
            try {
                ...
                const tokens = await oauthService.handleCallback(provider, code);
                const { agentId } = oauthService.parseState(state);
                const agent = await repo.agentRepository.findById(agentId);
                if (!agent)
                    throw new Error('Agent not found');
                await repo.connectionRepository.create(agent.tenantId, agentId, provider, 'oauth2.0', tokens);
    ...
    setupSessionRoutes() {
        this.app.post('/v1/ai/sessions/create', authenticateAgent, rateLimit(60), async (req, res) => {
            try {
                ...
                const session = await repo.sessionRepository.create(agent.id, agent.tenantId, context?.user_id || 'anonymous', tools || [], sandbox_config);
        ...
        this.app.post('/v1/ai/tools/discover', authenticateAgent, async (req, res) => {
            try {
                ...
                const dbAgent = {
                    ...
                    description: '',
                    status: 'active',
                    permissions: (dbAgentRecord?.permissions || {}),
    ...
    setupConnectionRoutes() {
        this.app.post('/v1/ai/connections/provision', authenticateAgent, async (req, res) => {
            try {
                ...
                const agent = req.agent;
                const connection = await repo.connectionRepository.create(agent.tenantId, user_id, provider, auth_type, tokens);
                res.json({
    ...
    setupRecipeRoutes() {
        this.app.post('/v1/ai/recipes/compile', authenticateAgent, async (req, res) => {
            try {
                ...
                const steps = (session.steps || []).filter((s) => s.status === 'completed').map((s, idx) => ({
                    step_number: idx + 1,
        ...
        this.app.get('/v1/adapters', authenticateAgent, async (req, res) => {
            try {
                ...
                if (search) {
                    ...
                    providers = providers.filter(p => p.name.toLowerCase().includes(lower) || p.description.toLowerCase().includes(lower));
                }
                res.json({
    ...
    async processMCPMessage(request) {
        try {
            let result;
    ...
    extractToolFromInstruction(instruction) {
        const lower = instruction.toLowerCase();
        if (lower.includes('slack') && (lower.includes('post') || lower.includes('send') || lower.includes('message')))
        ...
        if (lower.includes('google drive') || lower.includes('drive'))
            return 'google_drive.read';
        if (lower.includes('dropbox') && (lower.includes('search') || lower.includes('find')))
            return 'dropbox.search_files';
        if (lower.includes('dropbox') && lower.includes('upload'))
            return 'dropbox.upload_file';
        if (lower.includes('salesforce') && lower.includes('create'))
            return 'salesforce.create';
        if (lower.includes('salesforce') && lower.includes('query'))
            return 'salesforce.query';
        if (lower.includes('stripe') && lower.includes('charge'))
            return 'stripe.create_charge';
        if (lower.includes('hubspot') && lower.includes('contact'))
            return 'hubspot.create_contact';
        if (lower.includes('shopify') && lower.includes('product'))
            return 'shopify.list_products';
        if (lower.includes('jira') && lower.includes('issue'))
            return 'jira.create_issue';
        if (lower.includes('csv') && lower.includes('convert'))
            return 'csv_converter.convert';
        return 'generic.execute';
    }
    async executeRealTool(toolName, params, agent) {
        const [provider, action] = toolName.split('.');
        ...
        const credentials = connection
            ? { type: 'oauth2', accessToken: (await repo.connectionRepository.getDecryptedTokens(connection.id))?.access_token }
            : { type: 'api_key' };
