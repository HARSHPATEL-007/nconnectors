...
import { encrypt, decrypt, generateApiKey, generateConnectionId, hashData, generateMerkleRoot } from '../services/crypto.js';
const CACHE_TTL = 300;
export const tenantRepository = {
    ...
    async findById(id) {
        ...
        if (tenant)
            await cacheSet(`tenant:${id}`, tenant, CACHE_TTL);
        return tenant;
    },
};
export const agentRepository = {
    ...
    async findByApiKey(apiKey) {
        ...
        if (agent)
            await cacheSet(`agent:apiKey:${apiKey}`, agent, CACHE_TTL);
        return agent;
    },
...
export const connectionRepository = {
    async create(tenantId, userId, provider, authType, tokens) {
...
export const sessionRepository = {
    async create(agentId, tenantId, userId, tools, sandboxConfig) {
        ...
        return prisma.session.create({
            data: {
                ...
                userId,
                status: 'running',
                context: { userId, tenantId, session_type: 'interactive' },
                toolsInjected: tools,
                currentStep: 0,
                sandboxConfig: (sandboxConfig || { cpu_quota: 2, ram_quota: 4096, timeout_seconds: 600, network_mode: 'filtered' }),
    ...
    async findById(id) {
        ...
        if (session)
            await cacheSet(`session:${id}`, session, 60);
        return session;
    },
    async addStep(sessionId, step) {
    ...
    async create(entry) {
        ...
        return prisma.auditEntry.create({
            data: {
                ...
                toolParameters: entry.tool_parameters,
                ...
                approvalTimestamp: entry.approval_timestamp,
...
export const escalationRepository = {
    async create(data) {
        return prisma.escalationCase.create({
            data: {
                ...
                dataAccessed: [],
                proposedParameters: data.proposedParameters,
    ...
    async resolve(id, decision, resolvedBy, digitalSignature) {
