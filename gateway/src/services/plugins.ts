...
abstract class BasePlugin {
  ...
  constructor(name: string, slot: number, description: string) {
    this.name = name;
    this.slot = slot;
    this.description = description;
  }

  abstract run(context: PluginContext): Promise<PluginResult | null>;
}
...
class AuthOptimizerPlugin extends BasePlugin {
  ...
  async run(context: PluginContext): Promise<PluginResult | null> {
    this.triggerCount++;
...
class PluginManager {
  private slots: Map<number, BasePlugin> = new Map([
  ...
  async runAll(context: PluginContext): Promise<PluginResult[]> {
