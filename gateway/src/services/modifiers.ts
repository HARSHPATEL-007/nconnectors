...
class SchemaModifierService {
  ...
  private dangerousFields: Set<string> = new Set([
  ...
  redactSchema(schema: Record<string, unknown>, agent?: Agent): Record<string, unknown> {
    ...
    if (redacted.properties) {
      const props = redacted.properties as Record<string, unknown>;
      ...
      if (typeof value === 'string') {
        ...
        for (const pattern of this.piiPatterns) {
          masked = masked.replace(pattern, (match) => {
            if (match.includes('@')) return '***@***.com';
            return '***-**-****';
          });
        }
        obj[key] = masked;
      } else if (typeof value === 'object' && value !== null) {
        this.maskPII(value as Record<string, unknown>);
      }
    }
  }
