...
class HITLService extends EventEmitter {
  ...
  assessRisk(toolName: string, params: Record<string, unknown>, agent: Agent): number {
    ...
    if (toolNameLower.includes('delete') || paramsStr.includes('delete')) score += 0.3;
    if (toolNameLower.includes('remove') || toolNameLower.includes('destroy')) score += 0.25;
    if (toolNameLower.includes('update_budget') || paramsStr.includes('budget')) score += 0.2;
    if (toolNameLower.includes('transfer') || paramsStr.includes('transfer')) score += 0.35;
    if (toolNameLower.includes('send') || toolNameLower.includes('post')) score += 0.1;
    if (toolNameLower.includes('share') || paramsStr.includes('share')) score += 0.15;

    const amount = this.extractAmount(params);
    if (amount > 5000) score += 0.4;
    else if (amount > 1000) score += 0.2;
    else if (amount > 100) score += 0.1;

    const itemCount = this.extractItemCount(params);
    if (itemCount > 500) score += 0.3;
    else if (itemCount > 100) score += 0.15;

    if (agent.autonomy_level === 'low') score += 0.1;
    if (agent.autonomy_level === 'high') score -= 0.05;

    const isApprovedTool = agent.approval_required_for.includes(toolName);
    if (isApprovedTool) score += 0.2;
  ...
  query(filters: { status?: string; risk_level?: string }): EscalationCase[] {
  ...
  getStats(): { total: number; pending: number; approved: number; rejected: number; escalated: number } {
  ...
  private getTimeoutHours(risk_level: RiskLevel): number {
    switch (risk_level) {
  ...
  private extractAmount(params: Record<string, unknown>): number {
  ...
  private extractItemCount(params: Record<string, unknown>): number {
  ...
  private assessImpact(tool_name: string, params: Record<string, unknown>): string {
    const toolNameLower = tool_name.toLowerCase();
    if (toolNameLower.includes('delete')) return 'Data will be permanently deleted';
    if (toolNameLower.includes('send')) return 'Message will be sent to external recipients';
    if (toolNameLower.includes('transfer')) return 'Funds will be transferred';
    if (toolNameLower.includes('update')) return 'Records will be modified';
    if (toolNameLower.includes('create')) return 'New records will be created';
    return 'Action will be performed';
  }

  private scheduleTimeoutCheck(escalation_id: string, hours: number): void {
